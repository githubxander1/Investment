   ...
   if(InpLogSignals && signal!=SIGNAL_TYPE_NONE)
     {
      ...
      string signal_info = "信号生成 - 综合信号: " + signal_text + ", WPR信号: " + wpr_text + ", 布林带信号: " + bb_text;
      LogInfo(signal_info);
     }
   ...
   if(InpLogTrades)
     {
      ...
      string price_info = "价格: " + DoubleToString(type==POSITION_TYPE_BUY ? tick.ask : tick.bid, Digits());
      price_info += ", 止损: " + (sl==0 ? "无" : DoubleToString(sl, Digits()));
      price_info += ", 止盈: " + (tp==0 ? "无" : DoubleToString(tp, Digits()));
      LogInfo(price_info);
     }
