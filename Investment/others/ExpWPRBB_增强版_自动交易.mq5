...
input double               InpSLMltp         =  1.5;                 // 止损倍数（当SL==-1时）
input double               InpTPMltp         =  2.5;                 // 止盈倍数（当TP==-1时） - 改进：止盈倍数大于止损倍数
input bool                 InpEnableCloseOpposite = true;            // 是否平掉相反持仓
input int                  InpMaxPositions   =  3;                   // 最大持仓数 - 改进：减少最大持仓数以控制风险
input double               InpRiskPercent    =  1.0;                 // 每次交易风险百分比
input int                  InpMaxConsecutiveLosses = 3;               // 最大连续亏损次数 - 新增：风险管理参数
input bool                 InpUseAdvancedFilter = true;              // 使用高级信号过滤 - 新增：信号过滤参数
//--- 日志参数
...
void OnTick()
  {
   ...
   //--- 额外安全检查：验证关键数组索引是否可用
   // 检查价格数据有效性
   if(PriceOpen(0) == EMPTY_VALUE || PriceOpen(1) == EMPTY_VALUE || PriceOpen(2) == EMPTY_VALUE)
   {
      LogError("价格数据无效，无法继续处理");
      return;
   }
   
   // 检查WPR指标数据有效性
   if(WPR(0) == EMPTY_VALUE || WPR(1) == EMPTY_VALUE || WPR(2) == EMPTY_VALUE)
   {
      LogError("WPR指标数据无效，无法继续处理");
      return;
   }
   
   // 检查布林带指标数据有效性
   if(BBUpper(0) == EMPTY_VALUE || BBLower(0) == EMPTY_VALUE || BBUpper(1) == EMPTY_VALUE || BBLower(1) == EMPTY_VALUE || HalfSizeBB(0) == EMPTY_VALUE)
   {
      LogError("布林带指标数据无效，无法继续处理");
      return;
   }
   ...
   //--- 综合信号（优化信号生成逻辑）
   ENUM_SIGNAL_TYPE signal = SIGNAL_TYPE_NONE;
   
   // 基础逻辑：两个指标信号一致时生成信号
   if(signal_wpr==signal_bb && signal_wpr!=SIGNAL_TYPE_NONE)
      signal = signal_wpr;
   // 增强逻辑：如果启用高级过滤，且WPR信号强烈时也可交易
   else if(InpUseAdvancedFilter && signal_wpr!=SIGNAL_TYPE_NONE && signal_bb==SIGNAL_TYPE_NONE)
   {
      // WPR强烈信号判断：超买超卖区域边界内进一步确认
      if((signal_wpr==SIGNAL_TYPE_LONG && WPR(1) <= oversold_wpr + 10) || 
         (signal_wpr==SIGNAL_TYPE_SHORT && WPR(1) >= overbought_wpr - 10))
      {
         signal = signal_wpr;
      }
   }
   ...
   if(InpLogSignals && signal!=SIGNAL_TYPE_NONE)
     {
      ...
      string signal_info = "信号生成 - 综合信号: " + signal_text + ", WPR信号: " + wpr_text + ", 布林带信号: " + bb_text + ", WPR值: " + DoubleToString(WPR(1), 2) + ", 价格: " + DoubleToString(PriceOpen(0), Digits());
      LogInfo(signal_info);
     }
   ...
   if(InpSignalsOnly)
     {
      // 添加明确的日志提示
      if(InpLogSignals && (signal_wpr!=SIGNAL_TYPE_NONE || signal_bb!=SIGNAL_TYPE_NONE))
      {
         LogInfo("【注意】当前处于'仅显示信号'模式，不会执行自动交易。要启用自动交易，请在EA参数设置中将InpSignalsOnly设置为false。");
      }
      return;
     }
   ...
   if(signal!=SIGNAL_TYPE_NONE)
   {
      ...
      {
         LogInfo("【风险管理】连续亏损次数达到限制 (" + IntegerToString(consecutive_losses) + "), 暂停交易");
         // 可以选择在此处平掉所有持仓或等待一段时间
         return;
      }
...
//| 根据风险百分比计算手数                                           |
//+------------------------------------------------------------------+
double CalculateLotsByRisk(const double risk_percent)
  {
   //--- 获取账户信息
   ...
   //--- 计算止损点数（使用布林带半宽度）
   int stop_loss_points = HalfSizeBB(0);
   if(stop_loss_points <= 0)
      stop_loss_points = 50; // 默认止损点数
   
   //--- 获取每点价值
   double point_value = SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_VALUE);
   double point_size = SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE);
   double point_worth = point_value / point_size; // 每点价值
   
   //--- 计算所需手数
   double lots_needed = risk_amount / (stop_loss_points * point_worth);
   
   //--- 校正手数
   return CorrectLots(lots_needed);
  }
...
bool CloseBuy(void)
  {
   ...
   if(total > 0 && InpLogTrades)
      LogInfo("开始平仓买入持仓，持仓数: ", IntegerToString(total));
   ...
   for(int i=total-1; i>=0; i--)
     {
      ...
      if(!trade.PositionClose(ticket,InpDeviation))
        {
         res=false;
         LogError("平仓买入订单失败，订单号: ", IntegerToString(ticket));
        }
      else if(InpLogTrades)
        {
         double profit = PositionGetDouble(POSITION_PROFIT);
         LogInfo("成功平仓买入订单，订单号: ", IntegerToString(ticket), ", 盈亏: ", DoubleToString(profit, 2));
        }
     }
   return res;
  }
...
bool CloseSell(void)
  {
   ...
   if(total > 0 && InpLogTrades)
      LogInfo("开始平仓卖出持仓，持仓数: ", IntegerToString(total));
   ...
   for(int i=total-1; i>=0; i--)
     {
      ...
      if(!trade.PositionClose(ticket,InpDeviation))
        {
         res=false;
         LogError("平仓卖出订单失败，订单号: ", IntegerToString(ticket));
        }
      else if(InpLogTrades)
        {
         double profit = PositionGetDouble(POSITION_PROFIT);
         LogInfo("成功平仓卖出订单，订单号: ", IntegerToString(ticket), ", 盈亏: ", DoubleToString(profit, 2));
        }
     }
   return res;
  }
...
bool OpenPosition(const string symbol_name,const ENUM_POSITION_TYPE type,const double volume,const string comment)
  {
   ...
   if(result)
   ...
   else
     {
      // 增加连续亏损计数
      consecutive_losses++;
      LogInfo("连续亏损次数: " + IntegerToString(consecutive_losses));
     }
   ...
   if(InpLogTrades)
     {
      ...
      string price_info = "价格: " + DoubleToString(type==POSITION_TYPE_BUY ? tick.ask : tick.bid, Digits()) + ", 止损: " + (sl==0 ? "无" : DoubleToString(sl, Digits())) + ", 止盈: " + (tp==0 ? "无" : DoubleToString(tp, Digits()));
      LogInfo(price_info);
     }
...
void TradeProcess(const ENUM_SIGNAL_TYPE signal)
  {
   ...
   if(TotalPositions() >= InpMaxPositions)
     {
      LogInfo("已达到最大持仓数限制: ", IntegerToString(InpMaxPositions));
      return;
     }
   ...
   if(signal==SIGNAL_TYPE_LONG)
     {
      //--- 平掉相反持仓
      if(InpEnableCloseOpposite && TotalSell() > 0)
        {
         LogInfo("平掉相反持仓（卖）");
         if(!CloseSell())
            LogError("平仓失败");
   ...
   if(signal==SIGNAL_TYPE_SHORT)
     {
      //--- 平掉相反持仓
      if(InpEnableCloseOpposite && TotalBuy() > 0)
        {
         LogInfo("平掉相反持仓（买）");
         if(!CloseBuy())
            LogError("平仓失败");
...
void LogInfo(string text)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text);
  }

void LogError(string text)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text);
  }

void LogInfo(string text, string val1)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1);
  }

void LogError(string text, string val1)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1);
  }

void LogInfo(string text, string val1, string val2)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2);
  }

void LogError(string text, string val1, string val2)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2);
  }

void LogInfo(string text, string val1, string val2, string val3)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3);
  }

void LogError(string text, string val1, string val2, string val3)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3);
  }

void LogInfo(string text, string val1, string val2, string val3, string val4)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4);
  }

void LogError(string text, string val1, string val2, string val3, string val4)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4);
  }

void LogInfo(string text, string val1, string val2, string val3, string val4, string val5)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4, val5);
  }

void LogError(string text, string val1, string val2, string val3, string val4, string val5)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4, val5);
  }

void LogInfo(string text, string val1, string val2, string val3, string val4, string val5, string val6)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4, val5, val6);
  }

void LogError(string text, string val1, string val2, string val3, string val4, string val5, string val6)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4, val5, val6);
  }

void LogInfo(string text, string val1, string val2, string val3, string val4, string val5, string val6, string val7)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4, val5, val6, val7);
  }

void LogError(string text, string val1, string val2, string val3, string val4, string val5, string val6, string val7)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, val1, val2, val3, val4, val5, val6, val7);
  }

void LogInfo(string text, int val1)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, IntegerToString(val1));
  }

void LogError(string text, int val1)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, IntegerToString(val1));
  }

void LogInfo(string text, int val1, int val2)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, IntegerToString(val1), ", ", IntegerToString(val2));
  }

void LogError(string text, int val1, int val2)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, IntegerToString(val1), ", ", IntegerToString(val2));
  }

void LogInfo(string text, int val1, int val2, int val3)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, IntegerToString(val1), ", ", IntegerToString(val2), ", ", IntegerToString(val3));
  }

void LogError(string text, int val1, int val2, int val3)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, IntegerToString(val1), ", ", IntegerToString(val2), ", ", IntegerToString(val3));
  }

void LogInfo(string text, double val1)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, DoubleToString(val1, Digits()));
  }

void LogError(string text, double val1)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, DoubleToString(val1, Digits()));
  }

void LogInfo(string text, double val1, double val2)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, DoubleToString(val1, Digits()), ", ", DoubleToString(val2, Digits()));
  }

void LogError(string text, double val1, double val2)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, DoubleToString(val1, Digits()), ", ", DoubleToString(val2, Digits()));
  }

void LogInfo(string text, double val1, double val2, double val3)
  {
   if(InpLogSignals || InpLogTrades)
      Print("【信息】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, DoubleToString(val1, Digits()), ", ", DoubleToString(val2, Digits()), ", ", DoubleToString(val3, Digits()));
  }

void LogError(string text, double val1, double val2, double val3)
  {
   if(InpLogErrors)
      Print("【错误】[", Symbol(), "][", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), "] ", text, DoubleToString(val1, Digits()), ", ", DoubleToString(val2, Digits()), ", ", DoubleToString(val3, Digits()));
  }
//+------------------------------------------------------------------+