...
for f in file_list:
    ...
    # 计算未来N日涨跌幅
    for day in day_list:
        table[f'{day}日后涨跌幅'] = table['收盘价_复权'].shift(0 - day) / table['收盘价_复权'] - 1
        table[f'{day}日后是否上涨'] = table[f'{day}日后涨跌幅'] > 0
        table[f'{day}日后是否上涨'] = table[f'{day}日后是否上涨'].fillna(value=False)
...
for signal, group in all_table.groupby('signal'):
    ...
    for i in day_list:
        if signal == 1:
            print(f'{i}天后涨跌幅大于0概率', '\t', float(group[group[str(i) + '日后涨跌幅'] > 0].shape[0]) / group.shape[0])
        elif signal == 0:
            print(f'{i}天后涨跌幅小于0概率', '\t', float(group[group[str(i) + '日后涨跌幅'] < 0].shape[0]) / group.shape[0])
