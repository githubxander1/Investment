...
class DataManager:
    ...
    def _init_database(self):
        ...
        # 创建索引
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_stock_datetime 
            ON minute_data(stock_code, datetime)
        ''')
        
        cursor.execute('''
            CREATE INDEX IF NOT EXISTS idx_datetime 
            ON minute_data(datetime)
        ''')
    ...
    def save_minute_data(self, df: pd.DataFrame, stock_code: str) -> int:
        """
        保存分时数据
        ...
        try:
            cursor.executemany('''
                ...
                (stock_code, datetime, open, close, high, low, volume, amount, avg_price, change_pct)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', records)
            conn.commit()
            logger.info(f"成功保存 {len(records)} 条分时数据: {stock_code}")
            return len(records)
    ...
    def get_minute_data(self, stock_code: str, start_date: str = None, 
        ...
        try:
            ...
            if not df.empty:
                ...
                logger.info(f"查询到 {len(df)} 条分时数据: {stock_code}, {start_date} ~ {end_date}")
            else:
                logger.warning(f"未查询到分时数据: {stock_code}, {start_date} ~ {end_date}")
    ...
    def import_from_csv(self, csv_path: str, stock_code: str) -> bool:
        """
        从CSV文件导入分时数据
        ...
        try:
            ...
            count = self.save_minute_data(df, stock_code)
    ...
    def batch_import_from_cache(self, cache_dir: str, stock_codes: List[str] = None) -> Dict[str, int]:
        ...
        Args:
            cache_dir: 缓存目录路径（如: cache/fenshi_data）
        ...
        for csv_file in csv_files:
            ...
            if len(parts) >= 1:
                ...
                if success:
                    # 统计导入数量
                    df = self.get_minute_data(stock_code)
                    results[stock_code] = len(df)
