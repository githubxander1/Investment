...
1. 获取系统分时均价 - 修正单位转换问题确保计算准确性
   均价:SUM(AMOUNT,0)/(SUM(VOL,0)*100);{VOL单位为手，乘以100转换为股，保证均价计算精准}
   当前价:CLOSE;

2. 核心功能：偏离度计算（反映当前价与均价的偏离程度）
   偏离度:(CLOSE-均价)/均价*100.00;{结果为百分比，保留2位小数}

3. 成交量分析 - 量比改为【数值显示】（关键修改：用ROUND保留2位小数，避免分数）
...
4. 调整坐标范围 - 优化小幅度偏离的显示效果
   DRAWNULL,COLORRED;{通过空值绘制强制系统重新计算坐标，避免偏离度波动被压缩}

5. 偏离度放大显示 - 增强视觉辨识度
   偏离度放大:偏离度*50,COLORRED,LINETHICK4;{红色粗线绘制，突出偏离趋势}

6. 信号生成 - 结合价格偏离与成交量（逻辑不变）
...
该模块实现了基于价格与均价偏离度、成交量分析的交易策略指标计算与分析功能。
