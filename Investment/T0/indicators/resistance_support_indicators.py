...
def plot_tdx_intraday(stock_code, trade_date=None, stock_name=None):
    try:
        ...
        fig.suptitle(f'{display_name} 分时图 - {trade_date}', fontsize=14, y=0.98)
        ...
        # plt.subplots_adjust(top=0.95)  # 移除这行以避免警告
        ...
        chart_filename = os.path.join(output_dir, f'{filename_stock_part}_{trade_date}_阻力支撑指标.png')
...
# 批量分析函数
def analyze_resistance_support_batch(stock_info, trade_date=None):
    """
    批量分析多个股票的阻力支撑指标
    ...
    for stock_code, stock_name in stock_info.items():
        ...
        if result_df is not None:
            ...
            csv_file = os.path.join(stock_data_dir, f'{filename_stock_part}_{trade_date}_阻力支撑指标.csv')
