...
## 项目概述
本投资项目是一个前后端分离的应用，使用React作为前端框架，Flask作为后端框架，SQLite作为数据库。项目提供了策略管理、持仓分析、交易记录和性能评估等功能。此外，还集成了AI助手功能，用于处理安卓自动化交易中的元素定位失败问题。
...
### AI助手环境
1. 配置千问API密钥：在`utils/ai_assistant.py`文件中设置`api_key`变量为您的千问API密钥
2. 确保已安装`openai`库：`pip install openai`
3. 确保已安装`uiautomator2`库：`pip install uiautomator2`
...
### 启动AI助手
1. 确保已配置千问API密钥
2. 在需要处理元素定位失败的代码中，调用`safe_click`或`safe_send_keys`方法，并传入任务描述参数
3. 当元素定位失败时，AI助手会自动截取屏幕并调用千问API识别元素位置，然后执行相应的操作
...
### AI助手开发
1. 扩展AI助手功能: 在`utils/ai_assistant.py`中添加新的方法，用于处理不同类型的UI操作
2. 调试AI助手: 在`pages/page_common.py`中查看`safe_click`和`safe_send_keys`方法的实现，了解如何调用AI助手处理元素定位失败
3. 自定义任务描述: 在调用`safe_click`或`safe_send_keys`方法时，传入详细的任务描述，以帮助AI助手更好地理解操作意图
...
5. 添加更多技术指标分析
6. 实现数据导入导出功能
7. 优化AI助手功能，提高元素识别准确率
8. 增加对更多类型UI操作的支持，如滑动、长按等
