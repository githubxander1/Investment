...
class CommonPage:
    ...
    def safe_click(self, element, timeout=3, task_description=""):
        try:
            if element.wait(timeout=timeout):
                ...
                # 使用AI助手处理元素定位失败的情况
                if task_description:
                    logger.info(f"使用AI助手处理任务: {task_description}")
                    self.ai_assistant.handle_element_not_found(self.d, task_description)
                    return True
                return False
    ...
    def safe_send_keys(self, element, text, timeout=3, task_description=""):
        try:
            if element.wait(timeout=timeout):
                element.send_keys(text)
                return True
            else:
                logger.warning("输入失败：元素不存在")
                # 使用AI助手处理元素定位失败的情况
                if task_description:
                    logger.info(f"使用AI助手处理任务: {task_description}")
                    # 这里需要实现AI助手处理输入的逻辑
