...
from Investment.THS.AutoTrade.utils.ai_assistant import AIAssistant
...
class THSPage:
    ...
    def click_back(self):
        ...
        self.common_page.safe_click(back_button, task_description="点击返回按钮")
        logger.info("点击返回按钮")

    # 交易入口页
    def click_trade_entry(self):
        trade_entry = self.d(resourceId='com.hexin.plat.android:id/title', text='交易')
        self.common_page.safe_click(trade_entry, task_description="点击交易按钮(外)")
    ...
    def click_holding_stock_entry(self): #持仓-入口处
        ...
        self.common_page.safe_click(operate_entry, task_description="点击持仓按钮(外)")
    ...
    def click_operate_entry(self,operation):
        ...
        if operation == '买入':
            ...
            self.common_page.safe_click(buy_entry, task_description="点击买入按钮(外)")
            logger.info("点击买入按钮(外)")
        elif operation == '卖出':
            sale_entry = self.d(resourceId='com.hexin.plat.android:id/menu_sale_text')
            self.common_page.safe_click(sale_entry, task_description="点击卖出按钮(外)")
    ...
    def click_holding_stock_button(self): # 持仓-里面
        ...
        self.common_page.safe_click(holding_button, task_description="点击持仓按钮(里)")
    ...
    def click_operate_button(self,operation):
        ...
        if operation == '买入':
            ...
            self.common_page.safe_click(buy_button, task_description="点击买入按钮(里)")
            logger.info("点击买入按钮(里)")
            return True
        elif operation == '卖出':
            sale_button = self.d(className='android.widget.TextView', text='卖出')
            self.common_page.safe_click(sale_button, task_description="点击卖出按钮(里)")
    ...
    def click_refresh_button(self):
        ...
        self.common_page.safe_click(refresh_button, task_description="点击刷新按钮")
        logger.info("点击刷新按钮")

    def search_stock(self, stock_name):
        stock_search = self.d(resourceId='com.hexin.plat.android:id/content_stock')
        self.common_page.safe_click(stock_search, task_description="点击股票搜索框")
        ...
        if clear.exists():
            self.common_page.safe_click(clear, task_description="清除股票代码")
            logger.info("清除股票代码")
        time.sleep(1)
        ...
        if clear.exists():
            ...
            if recycler_view.wait(timeout=5):  # 等待最多5秒
                ...
                self.common_page.safe_click(first_item, task_description="点击匹配的第一个股票")
                ...
                if stock_code:
                    ...
                    if recycler_view.wait(timeout=5):
                        first_item = recycler_view.child(index=0)
                        self.common_page.safe_click(first_item, task_description="点击匹配的第一个股票（使用代码搜索）")
    ...
    def input_volume(self, volume):
        ...
        self.common_page.safe_send_keys(volumn_input, volume, task_description=f"输入数量: {volume}手")
    ...
    def click_half_volume(self):
        ...
        self.common_page.safe_click(volumn, task_description="输入数量: 半仓")
        logger.info("输入数量: 半仓")
    ...
    def click_total_volume(self):
        ...
        if volume.get_text() != '0':
            self.common_page.safe_click(volumn, task_description="输入数量: 全仓")
            logger.info("输入数量: 全仓")
        else:
            pass
    ...
    def click_submit_button(self, operation):
        ...
        self.common_page.safe_click(submit_button, task_description=f"点击按钮: {operation} (提交)")
