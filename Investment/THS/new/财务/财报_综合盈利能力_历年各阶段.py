...
def stock_diagnosis_request():
    url = "http://dq.10jqka.com.cn/fuyao/stock_diagnosis/finance/v2/ablility_history?type=stock&industry_type=&ability_id=profit&code=002836&market=33"
    headers = {
    ...
    try:
        ...
        return response.json()  # 如果响应是JSON格式，返回解析后的JSON数据
...
def extract_data(response_data):
    ...
    for item in details:
        ...
        extracted.append({
            ...
            "日期": report.get("date"),
            "盈利能力评分": float(item.get("score", 0)),
            "行业排名": f"{item.get('rank')}/{item.get('total')}",
            "百分比排名": round(float(item.get("percent", 0)) * 100, 2),
...
def display_and_save(data, filename="stock_profit_history.csv"):
    """
...
if __name__ == "__main__":
    response = stock_diagnosis_request()
