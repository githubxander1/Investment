...
def process_portfolio_ids(portfolio_ids):
    ...
    for portfolio_id in portfolio_ids:
        info = get_position_industry_info(portfolio_id)
        print('info信息')
        pprint(info)
        if info:
            for item in info:
            ...
            print(f"无法检索数据: {portfolio_id}")

    df = pd.DataFrame(all_data)
    df.to_excel(r'D:\1document\1test\PycharmProject_gitee\others\量化投资\THS\组合\保存的数据\持仓行业信息.xlsx', index=False)
