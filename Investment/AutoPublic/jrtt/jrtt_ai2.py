...
def generate_article_with_qwen(topic):
    ...
    try:
        ...
        print(f"使用AI生成文章失败: {e}")
        return None, None

def send_article_with_qwen(page):
    """使用Qwen大模型生成文章并发送"""
    # 继续执行其他操作
    ...
    try:
        ...
        # topics = generate_article_with_qwen(topic)
        if not topics:
    ...
    # 尝试使用Qwen生成完整文章
    ai_title, ai_content = generate_article_with_qwen(topic)

    if ai_title and ai_content:
        # 如果AI成功生成了文章，则使用AI生成的内容
        print("使用Qwen大模型生成的文章")
        # 填充标题
        page.get_by_role("textbox", name="请输入文章标题（2～30个字）").fill(ai_title)

        # 填充正文内容
        # 定位正文编辑器
        # content_editor = page.locator("#quillEditor div").first
        content_editor = page.locator("div").filter(has_text=re.compile(r"^请输入正文$"))
        if content_editor.is_visible():
            content_editor.fill(ai_content)
        else:
            # 备选方案
            page.get_by_role("textbox", name="请输入正文").fill(ai_content)
    else:
        # 如果AI生成失败，则使用原来的流程
        print("使用平台AI生成文章")
        # 主题
    ...
    # 继续后续的发布流程...
    # 点击添加到正文按钮（如果使用了平台AI）
    if not (ai_title and ai_content):  # 只有在使用平台AI时才需要点击添加到正文
        add_to_content = page.get_by_text("添加到正文")
        if add_to_content.is_visible(timeout=10000):
