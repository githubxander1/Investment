...
r3=re.match('(http|https)://([0-9a-zA-Z.-]+)(:[0-9]+)?(/.*)?',url)
#(http∣https)表示匹配http或https，://表示必须包含://，([a−zA−Z0−9.−]+)表示匹配一个或多个字母、数字、点号或短横线，(:[0−9]+)?表示端口号部分可选，可以是一个冒号加一个或多个数字，(/.∗)?表示路径部分可选，可以是一个斜杠加任意字符，表示匹配字符串的结束
print(r3)
