...
def save_results_to_xlsx(holding_data, sheet_name, filename):
    try:
        with pd.ExcelWriter(filename, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
            holding_data.to_excel(writer, sheet_name=sheet_name, index=False)
            print(f"持仓结果已保存到 {filename} 的 {sheet_name} 表")
...
if __name__ == '__main__':
    ...
    save_results_to_xlsx(final_df, 'ETF持仓', file_path)
    ...
    # 保存为Excel文件
    save_results_to_xlsx(final_df, '股票持仓', file_path)
