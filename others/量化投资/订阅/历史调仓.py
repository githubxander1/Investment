...
def process_ids(ids):
    ...
    df = df[['code', '策略名称', '策略描述', 'currentRatio', 'finalPrice', 'name', 'newRatio', 'createAt', 'portfolioId']]
...
def process_ids_for_new_df(ids):
    ...
    df = pd.DataFrame(all_data)
    df = df[['portfolioId', 'code', '策略名称', '策略描述', 'name', 'createAt', 'currentRatio', 'finalPrice', 'newRatio', 'content']]
    df.sort_values(by='createAt', ascending=False, inplace=True)
    df.drop_duplicates(subset=['portfolioId'], keep='first', inplace=True)
    print('新历史调仓：')
    pprint(df)
