								...
								<table class="form_teble_box add_border">
									...
									<tbody>
										...
										<tr>
											...
											<td class="t_field_value"><input class="ui-textbox" name="sub_name" value="<?php echo $this->_var['deal_info']['sub_name']; ?>"/>&nbsp;<span class="t_tip">[用于邮件，短信，列表展示的显示，字数不得超过20个字]</span></td>
										</tr>	
										<tr>
...
?>
														<option value="<?php echo $this->_var['row']['id']; ?>" <?php if ($this->_var['deal_info']['brand_id'] == $this->_var['row']['id']): ?>selected=selected<?php endif; ?>><?php echo $this->_var['row']['name']; ?></option>
    ...
    foreach ($_from AS $this->_var['row']):
?>
														<option value="<?php echo $this->_var['row']['id']; ?>" <?php if ($this->_var['deal_info']['shop_cate_id'] == $this->_var['row']['id']): ?>selected=selected<?php endif; ?>><?php echo $this->_var['row']['title_show']; ?></option>
													<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
												</select>
											</td>
										</tr>
...
?>">
										...
										<tr>
											<td class="t_field_name">添加关联销售商品:</td>
											<td class="t_field_value">
												<div class="add_deal_btn">
													<div class="btn_box f_l">
														<button class="ui-button add_related_deal" rel="white" type="button"">选择商品</button>
													</div>
													<span class="t_tip f_l">[ 添加后可以在商品购买页，同时显示 ]</span>
												</div>
											</td>
										</tr>
										<tr>
											<td class="t_field_name">已关联的商品:</td>
								...
								<table class="form_teble_box add_border">
									...
									<tbody>
										<tr>
											...
											<td class="t_field_value"><input class="ui-textbox" name="begin_time" value="<?php echo $this->_var['deal_info']['begin_time']; ?>" readonly="readonly" /><a href="javascript:void(0);" class="clear_input_btn " title="清空时间">清空</a><span class="t_tip">[不设置表示商品开始无限制]</span></td>
										</tr>	
										<tr>
											<td class="t_field_name">结束时间:</td>
											<td class="t_field_value"><input class="ui-textbox" name="end_time" value="<?php echo $this->_var['deal_info']['end_time']; ?>"/><a href="javascript:void(0);" class="clear_input_btn " title="清空时间">清空</a><span class="t_tip">[不设置表示商品结束无限制]</span></td>
										</tr>	
										<tr class="max_bought_row">
											<td class="t_field_name">库存数:</td>
											<td class="t_field_value"><input class="ui-textbox" name="max_bought" value="<?php echo $this->_var['deal_info']['max_bought']; ?>"/><span class="t_tip">[库存数 -1为无限]</span></td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td class="t_field_value">
												<select class="ui-select filter_select medium" name="is_refund" >
													<option value="1" <?php if ($this->_var['deal_info']['is_refund'] == 1): ?>selected="selected"<?php endif; ?>>是</option>
													<option value="0" <?php if ($this->_var['deal_info']['is_refund'] == 0): ?>selected="selected"<?php endif; ?>>否</option>
												</select>
											</td>
										</tr>
										...
										<tr>
											...
											<td class="t_field_value">
												<select class="ui-select filter_select medium" name="is_delivery">
													<option value="1" <?php if ($this->_var['deal_info']['is_delivery'] == 1): ?>selected="selected"<?php endif; ?>>是</option>
													<option value="0" <?php if ($this->_var['deal_info']['is_delivery'] == 0): ?>selected="selected"<?php endif; ?>>否</option>
												</select>	
											</td>
										</tr>
										<tr id="filter_row">
...
?>
					...
					<div class="confirm_form_btn">
						<input type="hidden" name="cache_free_delivery" value='<?php echo $this->_var['cache_free_delivery']; ?>'/>
						<input type="hidden" name="cache_deal_payment" value='<?php echo $this->_var['cache_deal_payment']; ?>'/>
						<input type="hidden" name="cache_deal_delivery" value='<?php echo $this->_var['cache_deal_delivery']; ?>'/>
					
					
						<input type="hidden" name="select_sub_cate" value="<?php echo $this->_var['select_sub_cate']; ?>"/>
						...
						<div class="sub_form_btn f_r">
							<?php if ($this->_var['allow_publish_verify'] == 1): ?>
								<button class="ui-button " rel="orange" type="submit">保存并发布</button>
							<?php else: ?>
