...
class AddNoteScreen(Screen):
    def __init__(self, name, app, **kwargs):
        ...
        # 内容
        self.content_input = TextInput(hint_text="请输入内容", multiline=True, size_hint_y=None, height=200,
                                       font_name=resource_find('simhei.ttf'))
        ...
        self.date_input = TextInput(text=self.selected_date, hint_text="请选择日期",
                                    font_name=resource_find('simhei.ttf'))
        ...
        self.tags_input = TextInput(hint_text="点击选择标签", readonly=True, on_focus=self.show_tag_popup,
                                    font_name=resource_find('simhei.ttf'))
    ...
    def show_tag_popup(self, instance, value):
        if value:  # 当输入框获得焦点时
            print("Tags input focused, showing tag popup...")
            self.create_and_show_tag_popup()

    def create_and_show_tag_popup(self):
        # 创建一个标签选择弹窗
