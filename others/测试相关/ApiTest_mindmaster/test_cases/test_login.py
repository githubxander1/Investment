...
from others.测试相关.ApiTest_mindmaster.common.yaml_handler import YamlHandler

# 定义测试类 TestLogin
yaml_handler= YamlHandler('../data/loginDataParameter.yaml')
test_data = yaml_handler.read_yaml()
# 初始化日志模块

# filename=os.path.basename(__file__).split('.')[0]
# logger = LoggerHandler(name=read_Yaml['logger']['name'],
#                        level=read_Yaml['logger']['level'],
#                        file=f'../log/{filename}_log.txt',
#                        format=read_Yaml['logger']['format'])
@allure.feature('登录')
class TestLogin:
    """登录用例"""
    ...
    @allure.story('登录用例')# 设置 allure.story 描述为 '登录用例'
    def test_login(self, items):
        ...
        # finally:
            ...
            # TestLogin.excel.write_excel('../testdata/openpyxl_mindmaster2.xlsx', 'login',items['case_id'] + 实例25_批量生成PPT版荣誉证书, 8, res['status'])
            # # 如果断言成功，则在第10行(测试结果)写入Pass,否则，写入Fail
            # TestLogin.excel.write_excel('../testdata/openpyxl_mindmaster2.xlsx', 'login',items['case_id'] + 实例25_批量生成PPT版荣誉证书, 9, result)
...
# 2.ddt(*testdata):* 表示对 case_data1 进行序列解包，将其作为独立的参数进行传递。
        # 也就是说，如果 case_data1 是一个列表或元组，这两种写法的效果一样。但是，如果 case_data1 是一个字典，则不能使用 @testdata(case_data1,) 这种写法。
        # 如果强制使用，会出现以下错误：TypeError: 'dict' object is not iterable
        # ddt(testdata,)表示将 case_data1 作为一个单独的元素放入元组中，并将该元组作为参数传递给装饰器,
        # 因此，在传递一个字典对象时，应该使用 @testdata(case_data1,) 的方式，而不是 @testdata(*case_data1)
# 3.字符串和json格式转换“
