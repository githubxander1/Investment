...
class TestLogin(unittest.TestCase):
    ...
    # 将字符串中的反斜杠替换为空
    # payload_str = case_data['testdata'].replace('\\', '')
    def setUp(self):
        # 请求类实例化
    ...
    def test_login(self, items):
        ...
        try:
            ...
            TestLogin.excel.write_excel('../testdata/openpyxl_mindmaster2.xlsx', 'login',items['case_id'] + 1, 8, res['status'])
            # 如果断言成功，则在第10行(测试结果)写入Pass,否则，写入Fail
            TestLogin.excel.write_excel('../testdata/openpyxl_mindmaster2.xlsx', 'login',items['case_id'] + 1, 9, self.result)
...
# 2.ddt(*testdata):* 表示对 case_data1 进行序列解包，将其作为独立的参数进行传递。也就是说，如果 case_data1 是一个列表或元组，这两种写法的效果一样。但是，如果 case_data1 是一个字典，则不能使用 @testdata(case_data1,) 这种写法。
