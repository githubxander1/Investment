...
def save_history_data():
    """将历史记录保存到本地文件"""
    # 将 BooleanVar 转换为布尔值
    history_data_serialized = [(time_str, row_num, stock_name, mode, base_price, calculated_price, price_difference, var if isinstance(var, bool) else var.get(), note) for time_str, row_num, stock_name, mode, base_price, calculated_price, price_difference, var, note in history_data]
    with open('history.json', 'w', encoding='utf-8') as f:
        json.dump(history_data_serialized, f, ensure_ascii=False, indent=4)
...
def load_history_data():
    ...
    try:
        with open('history.json', 'r', encoding='utf-8') as f:
            ...
            history_data = [(time_str, row_num, stock_name, mode, base_price, calculated_price, price_difference, tk.BooleanVar(value=value), note) for time_str, row_num, stock_name, mode, base_price, calculated_price, price_difference, value, note in loaded_data]
            return history_data
...
def calculate_profit_loss():
    try:
        ...
        if mode == '正T':
            ...
            history_data.append((datetime.now().strftime("%H:%M:%S"), len(history_data) + 1, stock_name, mode, base_price, sell_price, price_difference, selected_var, ""))
            save_history_data()
            update_history_table()
        elif mode == '倒T':
            # 计算买入价
            ...
            # 添加到历史记录
            selected_var = tk.BooleanVar()
            history_data.append((datetime.now().strftime("%H:%M:%S"), len(history_data) + 1, stock_name, mode, base_price, buy_price, price_difference, selected_var, ""))
            save_history_data()
            update_history_table()
        else:
...
label_stock_name.grid(row=0, column=0, padx=10, pady=10)
entry_stock_name = tk.Entry(root)
entry_stock_name.grid(row=0, column=1, padx=10, pady=10)

# 创建下拉框选择模式
mode_options = ['正T', '倒T']
combo_mode = ttk.Combobox(root, values=mode_options)
combo_mode.set('正T')
combo_mode.grid(row=1, column=1, padx=10, pady=10)

label_mode = tk.Label(root, text="模式：")
label_mode.grid(row=1, column=0, padx=10, pady=10)

label_base_price = tk.Label(root, text="基础价格:")
label_base_price.grid(row=2, column=0, padx=10, pady=10)
entry_base_price = tk.Entry(root)
entry_base_price.grid(row=2, column=1, padx=10, pady=10)

label_price_change = tk.Label(root, text="涨跌幅(%)：")
label_price_change.grid(row=3, column=0, padx=10, pady=10)
entry_price_change = tk.Entry(root)
entry_price_change.grid(row=3, column=1, padx=10, pady=10)
...
button_calculate.grid(row=6, column=1, padx=10, pady=10)

# 创建表格
treeview = ttk.Treeview(root, columns=("序号", "时间", "品种名称", "模式", "基础价格", "计算结果", "差价", "操作"), show="headings")
...
treeview.grid(row=5, column=0, columnspan=2, sticky="nsew")

# 创建单选框和备注输入框的框架
checkframe = tk.Frame(root)
checkframe.grid(row=5, column=2, sticky="nsew")
...
def update_history_table():
    ...
    for i, data in enumerate(history_data):
        ...
        # 添加删除按钮
        delete_button = tk.Button(checkframe, text="删除", command=lambda idx=i: delete_record(idx))
        delete_button.grid(row=i, column=2, sticky='w')
