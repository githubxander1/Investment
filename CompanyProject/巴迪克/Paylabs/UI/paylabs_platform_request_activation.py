...
def platform_request_activation(page,merchant_id):
    ...
    Request_Activation_button = corresponding_right_row.get_by_text("Request Activation")
    if merchant_status_content == "Request Activation":
        # with page.expect_popup() as popup_info:
        Request_Activation_button.click()
        ...
        #刷新页面
        page.reload()
        merchant_status1 = row.locator("td").nth(16)
        merchant_status_content = merchant_status1.text_content()
        if merchant_status_content == "Pending Activation Audit":
            print("✅资料提交成功，下一步：Pending Activation Audit")
        else:
            print(f"⚠️提示：商户状态不是Pending Activation Audit，请重新提交")
    else:
        print("⚠️提示：商户状态不是Request Activation，不需要再提交")