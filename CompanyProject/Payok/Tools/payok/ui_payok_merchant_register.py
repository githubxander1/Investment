...
from CompanyProject.Payok.Tools.payok.get_email_code import get_email_code

def payok_merchant_register(playwright: Playwright, login_email) -> None:
    ...
    upload_file(agreement, "1")# 上传营业执照
    upload_file(npwp, "2")# 上传纳税人号码（NPWP）
    upload_file(passport, "3")# 上传法人（责任人）护照
    ...
    page.get_by_role("textbox", name="业务归属地 * 邮箱 *").fill(login_email)
    ...
    try:
        continue_register = page.get_by_role("button", name="继续注册")
...
if __name__ == '__main__':
    login_email = "7@qq.com" # 登录邮箱
    with sync_playwright() as playwright:
        payok_merchant_register(playwright,login_email)
